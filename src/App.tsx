...
const DayEntry = lazy(() => import("./pages/DayEntry"));
...
const mobileTabItems = [
  ...
  { to: "/entry", label: "Today", icon: BookOpen },
  { to: "/calendar", label: "Calendar", icon: CalendarDays },
...
function App() {
  ...
  const [isLocked, setIsLocked] = useState(() => {
    ...
    return enabled && hash.length >= 60
  })
  const [pinDigits, setPinDigits] = useState(['','','',''])
  const [pinError, setPinError] = useState('')
  const [pinAttempts, setPinAttempts] = useState(() => {
    const value = Number(localStorage.getItem("pin_attempts") ?? "0");
    return Number.isFinite(value) ? value : 0;
  })
  const [pinLockedUntil, setPinLockedUntil] = useState(() => {
    const value = Number(localStorage.getItem("pin_locked_until") ?? "0");
    return Number.isFinite(value) ? value : 0;
  })
  const [pinShake, setPinShake] = useState(false)
  const [pinReady] = useState(true)
  const [lockCheckTime] = useState(() => Date.now())

  const isPinTemporarilyLocked = pinLockedUntil > lockCheckTime;
  ...
  const handleUnlock = async () => {
    if (isPinTemporarilyLocked) {
      setPinError("PIN temporarily locked. Please wait and try again.");
      return;
    }
    ...
    if (hashed === stored) {
      ...
      setPinAttempts((p) => {
        const next = p + 1;
        localStorage.setItem("pin_attempts", String(next));
          ...
          <div className="card text-center"
            ...
            {isPinTemporarilyLocked ? (
              <p style={{ color: 'var(--danger)' }}>
                Too many attempts ({pinAttempts}/{MAX_PIN_ATTEMPTS}). Try again later.
              </p>
            ) : (
              <>
